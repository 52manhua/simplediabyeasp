...
Class EasyAsp_db
	...
	Public Function wGetRecord(ByVal TableName,ByVal Condition,ByVal OrderField)
		...
		If OrderField <> "" Then strSelect = strSelect & " Order By " & OrderField
			'''Debug
			'Response.Write "GR: 分页 sql: " & strSelect & "<br>"
	...
	Public Function GetPageRecord(ByVal PageSetup, ByVal Condition)
		...
		''PageSetup 第一个参数是类型
		pType = o(0)
		'Response.write "pType =(" & o(1) & "}" & "<br>"
		If Not Easp_isN(o(1)) Then
			p = Easp_Param(Cstr(o(1)))
			''含有2个参数
			If Not Easp_isN(p(1)) Then
			'Response.write "GPR: run p=("  & p(0) & "," & p(1) &")<br>"				
		...
		'Response.write "GRP: 当前页 =("  & iPageIndex &")<br>"	
		Select Case Lcase(pType)
			Case "array","0"
				If isArray(Condition) Then
					'''调试
					'Response.write "---------GRP---------<br>"
					'Response.write "表和记录数量:" & Condition(0) & "<br>"
					'Response.write "条件:" & Condition(1) & "<br>"
					'Response.write "排序依据:" & Condition(2) & "<br>"
					'Response.write "主键:" & Condition(3) & "<br>"
					'Response.write "不常用:" & Condition(4) & "<br>"
					'Response.write "---------------------<br>"
					...
					'''分页页码的计算
					''
					'Response.write "GRP: iPageSize= " & iPageSize & " iPageCount= " & iPageCount & " iPageIndex=" & 	iPageIndex & " iRecordCount= " & iRecordCount & "<br>"
								
					If idbType = "1" or idbType = "ACCESS" Then
						''使用 {table}:n  的方法处理 access 数据库
						''针对 ACCESS 的情况 使用 AbsolutePage 进行分页
						'Response.write "GPR: access:" & Table&":"&Fi & Where & Condition(2) & "<br>" 
	...
	Public Function Pager(ByVal PagerHtml, ByRef PagerConfig)
		'''对  Recordset 的 调试
		'Response.write "空变量?: " & Easp.IsN(iRecordCount) & iRecordCount
		On Error Resume Next
...
Function Easp_Param(ByVal s)
	'Response.write "param: " & typename(s) & "<br>"
